...
typedef enum {
	...
	T_openparen,	// 7: (
	T_closeparen, 	// 8: )
	T_newline       // 9: newlines to keep track of lineNum
} token_type;
...
public:
  ...
  void eraseNums(vector <token_type> v);
...
void scanner_t::eat_token(token_type c)
{
  ...
  if(c != this->tokens[0]){
    ...
    this->tokens.erase(tokens.begin());
  }
...
void scanner_t::eraseNums(vector<token_type> v){
  int size = v.size() - 1;
  int i = 0;
  while(i < size){
    cout << "tokenToString(v[i]) " << token_to_string(v[i]) << "\n";
    cout << "tokenToStringv([i+1]) " << token_to_string(v[i+1]) << "\n";
    
    if((token_to_string(v[i]) == token_to_string(T_num)) && (token_to_string(v[i+1]) == token_to_string(T_num))){
...
void parser_t::List()
{
  ...
  if(scanner.next_token() == T_newline){
    eat_token(T_newline);
    scanner.lineNum++;
    while(scanner.next_token() == T_newline){
      eat_token(T_newline);
      scanner.lineNum++;
    }
  }
...
void parser_t::ExprP(){
  ...
  switch(scanner.next_token())
    ...
    default:
      if(scanner.next_token() == T_eof || scanner.next_token() == T_period || scanner.next_token() == T_closeparen){
	parsetree.drawepsilon();
	break;
      }
      else{
...
void parser_t::TermP(){
  ...
  switch(scanner.next_token())
    ...
    default:
      if(scanner.next_token() == T_plus || scanner.next_token() == T_minus || scanner.next_token() == T_period || scanner.next_token() == T_closeparen){
	parsetree.drawepsilon();
	break;
      }
      else{
...
int main(){
  ...
  return 0;
    ...
    /*for(int x = 0; x < parser.scanner.tokens.size() - 1; x++){
    cout << token_to_string(parser.scanner.tokens[x]) << "\n";
    }*/
}
